...
def tuned_matplotlib_process(fft_queue, quit, update_params, rate, center_freq, gain, fft_size):
    class Index:
        ...
        def change_freq(self, freq):
            if freq != '':
                ...
                self.xf = set_xf(self.ax, 1000, rate.value//2, center_freq.value)
...
def fft_process(sdr_queue, fft_queue, tuned_fft_queue, quit, fft_size):
    window = windows.hann(fft_size)
    window2 = windows.hann(1000)
...
#    pool = multiprocessing.Pool(processes=2)
    while quit.is_set() is False:
        try:
            ...
            # ratio = 2
            # data = data[::ratio]
            # tuned_data = np.array(data, copy=True)
            ...
            # print(tuned_data[::2])
            # tuned_data = tuned_data[::3]
            # print(data.size)
            # print(tuned_data.size)
            # print(tuned_data.size//fft_size)
            # print(fft_size)
            # try:
            #     tuned_data.resize(tuned_data.size//fft_size, fft_size)
            # except:
            #     print("prob")
            ...
            # for i in range(tuned_data.size//1000):
            #     mod_sig = sig.slice(700000, 1000)
            #     tuned_data[i * 1000: (i + 1) * 1000] = get_fft(tuned_data[i * 1000: (i + 1) * 1000] * window2 / mod_sig)
            # for i in range(tuned_data.size//fft_size):
            #     mod_sig = sig.slice(700000, fft_size)
            #     tuned_data[i * fft_size: (i + 1) * fft_size] = get_fft(tuned_data[i * fft_size: (i + 1) * fft_size] * window / mod_sig)
            # tuned_data = tuned_data.mean(axis=0)
            # try:
            #     tuned_fft_queue.put_nowait(data)
            # except:
            #     print("fail")
            #     pass
        except:
...
if __name__ == "__main__":
    ...
    tuned_fft_queue = multiprocessing.Queue(8)
    ...
    fft_process_list = [multiprocessing.Process(None, fft_process, args=(sdr_queue, fft_queue, tuned_fft_queue, quit, fft_size)) for _ in range(1)]

    for proc in fft_process_list:
        proc.start()

    run_tuned_matplotlib_process=multiprocessing.Process(None, tuned_matplotlib_process, args=(tuned_fft_queue, quit, update_params, rate, center_freq, gain, fft_size))
    run_tuned_matplotlib_process.start()
